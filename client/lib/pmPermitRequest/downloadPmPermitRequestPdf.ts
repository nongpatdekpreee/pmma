...
import type { PmPermitRequestData } from './types';
import {
...
let pmPermitExportCssCache: string | null = null;

async function loadPmPermitRequestExportCss(): Promise<string> {
  const maCss = await loadMaWorkOrderExportCss();
  if (pmPermitExportCssCache) return `${maCss}\n${pmPermitExportCssCache}`;
  const res = await fetch('/pm-permit-request/export.css');
  if (!res.ok) throw new Error('Failed to load PM permit request export styles');
  pmPermitExportCssCache = await res.text();
  return `${maCss}\n${pmPermitExportCssCache}`;
}
...
async function renderDocumentInIsolatedIframe(
  data: PmPermitRequestData,
  exportCss: string
): Promise<{ iframe: HTMLIFrameElement; cleanup: () => void }> {
  const { PmPermitRequestDocument } = await import('@/components/pm-permit-request/PmPermitRequestDocument');
  ...
  await new Promise<void>((resolve) => {
    root.render(React.createElement(PmPermitRequestDocument, { data }));
...
export async function downloadPmPermitRequestPdf(
  data: PmPermitRequestData,
  filename: string
): Promise<void> {
  await ensureMaWorkOrderFonts();
  const exportCss = await loadPmPermitRequestExportCss();
  ...
  const rootEl = iframeDoc.querySelector('#pm-permit-request-document');
  const pages = rootEl
    ? [...rootEl.querySelectorAll('.pm-pr-page')]
    : [...iframeDoc.querySelectorAll('.pm-pr-page')];
