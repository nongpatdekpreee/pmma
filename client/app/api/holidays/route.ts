...
export async function GET(req: Request) {
  try {
    ...
    const customList: HolidayItem[] = JSON.parse(holidaysData);
    ...
    return NextResponse.json({ success: false, message: e?.message || 'Failed to load holidays' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const list: HolidayItem[] = JSON.parse(data);
    ...
    return NextResponse.json({ success: false, message: e?.message || 'Failed to add holiday' }, { status: 500 });
  }
}
